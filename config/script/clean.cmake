...
file(GLOB_RECURSE DEV_ENTRIES LIST_DIRECTORIES True RELATIVE "${DEV_ROOT_DIR}" "${DEV_ROOT_DIR}/*")
list(SORT DEV_ENTRIES ORDER DESCENDING)

# Remove files and directories
foreach (DEV_ENTRY ${DEV_ENTRIES})
    if (NOT "${DEV_ENTRY}" MATCHES "^DEBIAN(/control)?$")
        set (DEV_DELETE True)
        ...
        endforeach()
        if (DEV_DELETE)
            message("-- Cleaning: ${DEV_ROOT_DIR}/${DEV_ENTRY}")
            if (NOT DEV_PRETEND)
                file(REMOVE_RECURSE "${DEV_ROOT_DIR}/${DEV_ENTRY}")
                if (EXISTS "${DEV_ROOT_DIR}/${DEV_ENTRY}")
                    message(WARNING "Could not remove ${DEV_ROOT_DIR}/${DEV_ENTRY}")
                endif()
            endif()
        endif()
    endif()
